...
{
	char *buffer = NULL, *buffer_copy = NULL, *trimmed_string = NULL,
	     *token_str = NULL;
	size_t n = 0;
	ssize_t num_chars;
	char *delim = " \n";
	...
	while (1)
	{
		...
		token_str = _strtok(trimmed_string, delim);
		while (token_str)
		{
			num_tokens++;
			token_str = _strtok(NULL, delim);
		}
		num_tokens++;
		...
		token_str = _strtok(buffer_copy, delim);
		i = 0;
		while (token_str)
		{
			token[i] = token_str;
